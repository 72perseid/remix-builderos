...
function KanbanBoard({ children, className }: KanbanBoardProps) {
  ...
  return (
    <SortableContext items={columnIds} strategy={rectSortingStrategy}>
      <div data-slot="kanban-board" className={cn('flex gap-4 overflow-x-auto pb-4', className)}>
        {children}
      </div>
    </SortableContext>
  );
  ...
  return (
    <ColumnContext.Provider value={{ attributes, listeners, isDragging: isSortableDragging, disabled }}>
      <div
        ...
        ref={setNodeRef}
        style={style}
        className={cn('flex flex-col', isColumnDragging && !isSortableDragging && 'opacity-50', className)}
      >
        {children}
      </div>
...
function KanbanColumnHandle({ asChild, className, children, cursor = true }: KanbanColumnHandleProps) {
  ...
  return (
    <Comp
      ...
      {...attributes}
      {...listeners}
      className={cn(!disabled && cursor && (isDragging ? 'cursor-grabbing' : 'cursor-grab'), className)}
    >
      {children}
    </Comp>
  );
...
function KanbanItem({ value, asChild = false, className, children, disabled }: KanbanItemProps) {
  ...
  return (
    <ItemContext.Provider value={{ listeners, isDragging: isSortableDragging, disabled }}>
      <Comp
        ...
        ref={setNodeRef}
        style={style}
        className={cn(isItemDragging && !isSortableDragging && 'opacity-50', className)}
        {...attributes}
        {...listeners}
      >
        {children}
...
function KanbanOverlay({ children, className }: KanbanOverlayProps) {
  ...
  return (
    <DragOverlay dropAnimation={dropAnimationConfig}>
      <div data-slot="kanban-overlay" className={cn(className)} style={style}>
        {content}
      </div>
    </DragOverlay>
  );
