...
AS $function$
DECLARE
  was_full INTEGER := CASE WHEN COALESCE(OLD.bm_completion,0)=100 AND COALESCE(OLD.pb_completion,0)=100 AND COALESCE(OLD.uv_completion,0)=100 AND COALESCE(OLD.ux_completion,0)=100 THEN 1 ELSE 0 END;
  is_full  INTEGER := CASE WHEN COALESCE(NEW.bm_completion,0)=100 AND COALESCE(NEW.pb_completion,0)=100 AND COALESCE(NEW.uv_completion,0)=100 AND COALESCE(NEW.ux_completion,0)=100 THEN 1 ELSE 0 END;
  artifacts jsonb := '[]'::jsonb;
  payload   jsonb := '{}'::jsonb;
BEGIN
  RAISE LOG 'check_completion_and_trigger: id=% old(%,%,%,%) new(%,%,%,%)', NEW.id,
    OLD.bm_completion, OLD.pb_completion, OLD.uv_completion, OLD.ux_completion,
    NEW.bm_completion, NEW.pb_completion, NEW.uv_completion, NEW.ux_completion;
